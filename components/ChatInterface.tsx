...
import { createChatSession, analyzePatientEmotion } from '../services/geminiService';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ user, onAnalysisComplete }) => {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: crypto.randomUUID(),
        role: 'model',
        text: "I'm having trouble connecting right now. Can we try that again?",
        timestamp: new Date()
      }]);
    } finally {
      setIsTyping(false);
  ...
  const handleEndSession = async () => {
    ...
    try {
      ...
      const report = await analyzePatientEmotion(transcript, user.name);
            ...
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-fade-in`}>
              <div className={`flex gap-3 max-w-[80%] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center shadow-sm ${
                  isUser ? 'bg-indigo-100' : 'bg-white border border-purple-100'
                }`}>
                  {isUser ? (
